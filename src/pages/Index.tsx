...
const Index = () => {
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case "search":
        // 내가 참여중인 챌린지명 목록
        const myRoutineNames = state.routines.map(r => r.routine);
        return (
          <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 pt-20 pb-24">
            <div className="max-w-3xl mx-auto px-4 py-8">
              <h2 className="text-2xl font-bold text-slate-800 mb-6">챌린지 탐색</h2>
              <div className="grid gap-6">
                {state.challenges.map((challenge) => {
                  const isJoined = myRoutineNames.includes(challenge.name);
                  return (
                    <div key={challenge.id} className="p-6 rounded-xl bg-white/90 shadow flex flex-col md:flex-row md:items-center md:justify-between gap-4 border border-slate-100">
                      <div>
                        <div className="text-lg font-bold text-indigo-700 mb-1">{challenge.name}</div>
                        <div className="text-slate-600 text-sm mb-1">카테고리: {challenge.category}</div>
                        <div className="text-slate-500 text-xs mb-2">시작: {challenge.startDate}</div>
                        <div className="flex gap-2 text-xs text-slate-400">
                          <span>�� {challenge.streak}일</span>
                          <span>참여자 {challenge.recentPosts}명</span>
                        </div>
                      </div>
                      <button
                        className={`px-5 py-2 rounded-lg font-semibold shadow transition ${isJoined ? 'bg-slate-200 text-slate-400 cursor-not-allowed' : 'bg-gradient-to-r from-indigo-500 to-purple-500 text-white hover:from-indigo-600 hover:to-purple-700'}`}
                        disabled={isJoined}
                      >
                        {isJoined ? '참여중' : '참여하기'}
                      </button>
